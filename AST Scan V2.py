...
keywords_raw = []
x = 0
#Assignment Data Extractor

def keywords_extractor(json_data):
    global keywords_raw
    global x
    if isinstance(json_data, dict):
        for key, value in json_data.items():
            ...
            #keywords_raw.append((key, value))
            else:
               keywords_extractor(value)

    elif isinstance(json_data, list):
        for item in json_data:
            keywords_extractor(item)



def Statement_Counter(json_data, key):
    global n_statement
    ...
    if isinstance(json_data, dict):
        for k, v in json_data.items():
            if k == key:
                ...
                for n, m in v.items():
                    if n == "kind" and m == "ExpressionStatement":
                                n_statement += 1
                                #print("If-Else block no. "+str(n_block)+" has "+str(n_statement)+" Statements") 
                    


            else:
                Statement_Counter(v, n_block, key)
    elif isinstance(json_data, list):
        for item in json_data:
            Statement_Counter(item, n_block, key)
        ...
        for k, v in json_data.items():
            if k == key:
                if v == "Conditional":
                    ...
                    #Statement_Counter(data, n_if_block , "ifTrue")
...
#Statement_Counter(data, "body")
keywords_extractor(data)
